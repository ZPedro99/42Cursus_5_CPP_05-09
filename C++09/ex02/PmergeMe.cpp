...
/*   Updated: 2023/12/27 12:04:59 by jomirand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	PmergeMe::sorting()
{
	...
	std::cout << "Time to process a range of " << _vectorArray.size() << " elements with std::vector : " << time_elapsedVec << " s" << std::endl;
	double time_elapsedDeq = static_cast<double>(endDeq - startDeq) / CLOCKS_PER_SEC;
	std::cout << "Time to process a range of " << _dequeArray.size() << " elements with std::deque : " << time_elapsedDeq << " s" << std::endl;
}
